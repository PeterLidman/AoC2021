...
public class L15b {
	...
	public static void run(String ext) throws IOException {
		...
		row = matrix.size();
		grid = new int[col * 5][row * 5];
		dist = new int[col * 5][row * 5];

		for (int y = 0; y < 5; y++) {
			for (int x = 0; x < 5; x++) {
				for (int j = 0; j < col; j++) {
					for (int i = 0; i < row; i++) {
						grid[i + (x * row)][j + (y * col)] = wrapValue(
								Integer.valueOf(matrix.get(j).substring(i, i + 1)) + x + y);
						dist[i + (x * row)][j + (y * col)] = Integer.MAX_VALUE;
					}
				}
			}
		}
	...
	private static int wrapValue(int in) {
		while (in > 9) {
			in -= 9;
		}
		return in;
